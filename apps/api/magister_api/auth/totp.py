"""RFC-6238 TOTP helpers for the local break-glass account (ADR-0015 D2).

Deliberately thin: :mod:`pyotp` does the RFC work, this module owns the policy
decisions around it — window, step bookkeeping, recovery-code shape, and the
provisioning URI plus its QR rendering.

Nothing here touches the database or logs a code; callers hold the row.
"""
...
def verify_code(secret: str, code: str, *, last_step: int | None) -> int | None:
    ...
    # like "０", and ``compare_digest`` raises TypeError on non-ASCII input —
    # which would turn a malformed code into a 500 instead of "invalid".
