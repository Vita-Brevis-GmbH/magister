...
This module deliberately does NOT enable the departments module, so it stays off
...
async def test_disabled_module_routes_return_404(as_schulleitung_a: AsyncClient) -> None:
    # departments is off by default -> its routes 404 with module_disabled.
...
async def test_enabled_module_routes_pass(as_schulleitung_a: AsyncClient) -> None:
    # classes is on by default -> its routes are reachable (guard passes).
