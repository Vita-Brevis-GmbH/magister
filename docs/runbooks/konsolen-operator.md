...
| **Code wird abgewiesen, mit jeder App** | Nicht das Telefon, sondern meist die Uhr. Einordnen: `python -m cockpit_api.cli.totp_probe --upn … --code <was die App zeigt>` sagt, ob der Code zu einem Zeitschritt passt (dann Uhr) oder zu keinem (dann hält die App ein anderes Geheimnis — neu einrichten). |
| **Telefon oder QR-Code verloren** | `add_operator --upn … --reset-mfa` löscht Geheimnis, Zeitschritt und alle Wiederherstellungscodes; die Person richtet den zweiten Faktor beim nächsten Anmelden neu ein. |
