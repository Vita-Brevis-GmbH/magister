...
# --- Strict 10-container topology (ADR-0011) ------------------------------------
#
# One container per non-platform module; the base ``magister-api`` stays the
# platform catch-all + the single migrator. The AD boundary is strict: only the
# ``ad`` container holds AD credentials and runs the sync loop; every other
# container is AD-credential-free and reaches AD through the ad container's RPC.

AD_OWNER = "ad"

# Base env for every split container — RPC-client role, NO AD credentials.
_STRICT_SHARED_ENV = """\
    MAGISTER_ENVIRONMENT: production
    MAGISTER_LOG_LEVEL: ${MAGISTER_LOG_LEVEL:-INFO}
    MAGISTER_DATABASE_URL: postgresql+asyncpg://${POSTGRES_USER:-magister}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-magister}
    ...
    MAGISTER_OIDC_CLIENT_SECRET: ${MAGISTER_OIDC_CLIENT_SECRET}
    MAGISTER_OIDC_REDIRECT_URI: https://${MAGISTER_PUBLIC_HOSTNAME:?MAGISTER_PUBLIC_HOSTNAME is required}/api/auth/callback
    # Strict AD boundary: no AD credentials here — reach AD only via the ad container.
    MAGISTER_AD_RPC_URL: http://magister-api-ad:8000
    MAGISTER_AD_RPC_SECRET: ${MAGISTER_AD_RPC_SECRET:?MAGISTER_AD_RPC_SECRET is required}
    MAGISTER_SKIP_MIGRATIONS: "1"
    MAGISTER_RUN_SCHEDULER: "0"\
"""

# The magister-api catch-all, reconfigured for the strict split: platform-only,
# still the single migrator (MAGISTER_SKIP_MIGRATIONS unset), an RPC client with
# no AD credentials and no scheduler. Overrides merge onto the base service env.
_MAIN_OVERRIDE = """\
  # Reconfigure the base catch-all for the strict split.
  magister-api:
    environment:
      MAGISTER_CONTAINER_MODULES: "platform"
      MAGISTER_RUN_SCHEDULER: "0"
      MAGISTER_AD_RPC_URL: http://magister-api-ad:8000
      MAGISTER_AD_RPC_SECRET: ${MAGISTER_AD_RPC_SECRET:?MAGISTER_AD_RPC_SECRET is required}
      MAGISTER_AD_DCS: ""
...
def strict_spec() -> dict[str, list[str]]:
    """One container per non-platform module; container name == module id."""
    return {mid: [mid] for mid in module_path_prefixes() if mid != "platform"}


def render_strict_caddy() -> str:
    """Caddy routing for the full 10-container strict split (platform = catch-all)."""
    return render_caddy_routes(strict_spec())


def render_strict_compose() -> str:
    """The full strict-split overlay: 9 module containers + the reconfigured base."""
    spec = strict_spec()
    lines = [
        "# GENERATED by scripts/gen_split.py --strict — do not edit by hand.",
        "# Strict 10-container split (ADR-0011): platform catch-all + one container",
        "# per module; only the ad container holds AD credentials + the sync loop.",
        "# Usage: docker compose -f docker-compose.yml -f <this-file> up -d",
        "# Requires MAGISTER_AD_RPC_SECRET in .env (shared internal bearer).",
        "",
        "x-split-api: &split-api",
        "  image: ${MAGISTER_API_IMAGE:-ghcr.io/vita-brevis-gmbh/magister-api:latest}",
        "  restart: unless-stopped",
        "  # Wait for the migrator (base magister-api) to be healthy → schema ready.",
        "  depends_on:",
        "    magister-api:",
        "      condition: service_healthy",
        "  environment: &split-env",
    ...
    for name in spec:
        ...
        lines.append("    environment:")
        lines.append("      <<: *split-env")
        lines.append(f'      MAGISTER_CONTAINER_MODULES: "{name}"')
        if name == AD_OWNER:
            lines.append("      # The one AD-capable container: real creds, owns the sync loop,")
            lines.append("      # blanks the RPC URL so it talks to AD directly.")
            lines.append('      MAGISTER_AD_RPC_URL: ""')
            lines.append('      MAGISTER_RUN_SCHEDULER: "1"')
            lines.append("      MAGISTER_AD_DCS: ${MAGISTER_AD_DCS:-}")
            lines.append("      MAGISTER_AD_BIND_DN: ${MAGISTER_AD_BIND_DN:-}")
            lines.append("      MAGISTER_AD_BIND_PASSWORD: ${MAGISTER_AD_BIND_PASSWORD:-}")
            lines.append("      MAGISTER_AD_USERS_SEARCH_BASE: ${MAGISTER_AD_USERS_SEARCH_BASE:-}")
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if tokens and tokens[0] == "--strict":
        ...
        print(render_strict_caddy())
        print("# ===== Compose overlay (deploy/compose/docker-compose.split10.yml) =====")
        print(render_strict_compose())
        return 0
    if not tokens:
        print(__doc__)
        return 2
